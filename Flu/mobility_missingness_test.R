...
social_distancing_county_network_unnorm_edgelist_daily_2021_NEng<- social_distancing_county_network_unnorm_edgelist_daily_2021 %>% filter(origin_county_fips %in% NEng_counties & destination_county_fips %in% NEng_counties)

# Split mobility data into monthly network slices– following Pullano et al. 2024 --------
social_distancing_county_network_unnorm_edgelist_month_NEng <- social_distancing_county_network_unnorm_edgelist_daily_2021_NEng %>% group_by(month(date)) %>% group_split()
...
flu_ts_NEng<- create_ts(county_shape = NEng_county_shape)[[1]] |> filter(rownames() %in% week(social_distancing_county_network_unnorm_edgelist_daily_2021_NEng))
